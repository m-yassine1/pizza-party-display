...
$pizzaSheet = fopen("PizzaSheet.csv", "r") or die("Unable to open file!");
$partySheet = fopen("PartyInformationSheet.csv", "r") or die("Unable to open file!");
...
while(!feof($pizzaSheet)) {
    $line = fgets($pizzaSheet);
    if(empty(trim($line))) {
        continue;
    }
...
while(!feof($partySheet)) {
    $line = fgets($partySheet);
    if(empty(trim($line))) {
        continue;
    }
